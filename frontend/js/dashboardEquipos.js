...
async function cargarEquipos() {
    try {
        const sessionRes = await fetch('/alarma_tec_1/back-end/get_session_user.php');
        const userData = await sessionRes.json();
        const url = `/alarma_tec_1/back-end/obtener_equipos_usuario.php?id_usuario=${userData.id_user}`;
...
function agregarEventos() {
    ...
    document.querySelectorAll('.btn-entrar').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            window.location.href = `/alarma_tec_1/frontend/dashboardZonas.html?id_equipo=${idEquipo}`;
        });
    });
